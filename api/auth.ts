...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!userString) return res.status(400).json({ error: 'User data not found' });
    ...
    res.status(200).json({ success: true, user: { id: user.id, telegramId: user.telegramId.toString() }, token });
  } catch (error: any) {
